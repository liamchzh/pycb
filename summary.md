...
------------------
###操作符重载
在Python中，重载__getattr__、__setattr__、__delattr__和__getattribute__方法可以用来管理一个自定义类中的属性访问。

* __getattr__:拦截所有未定义的属性
* __getattribute__:不管属性是否已定义，都会调用
* __setattr__:拦截所有的属性赋值
* __del__:拦截所有的属性删除
